...
if ($userDetails['view_permission'] == "Yes") {
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (customer_master.name like '%" . $searchValue . "%' or 
        customer_master.ph_num like '%" . $searchValue . "%' or 
        customer_master.address like'%" . $searchValue . "%' ) ";
    }

    $query = "select 
    customer_master.customer_code,
    customer_master.name,
    customer_master.ph_num,
    ...
    while ($row = mysqli_fetch_assoc($empRecords)) {
        ...
        $data[] = array(
            "name" => '<input type="hidden" class="customer_code_' . $i . '" value="' . $row['customer_code'] . '" />' . $row['name'],
            "ph_num" => $row['ph_num'],
            "address" => $row['address'],
            "action" => $action,
        );
