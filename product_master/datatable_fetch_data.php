...
if ($userDetails['view_permission'] == "Yes") {
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (product_master.name like '%" . $searchValue . "%' or 
        product_master.category like '%" . $searchValue . "%') ";
    }
    ...
    while ($row = mysqli_fetch_assoc($empRecords)) {
        ...
        $data[] = array(
            "name" => '<input type="hidden" class="product_code_' . $i . '" value="' . $row['product_code'] . '" />' . $row['name'],
