...
if ($userDetails['view_permission'] == "Yes") {
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (unit_master.name like '%" . $searchValue . "%' or 
        unit_master.active like '%" . $searchValue . "%' )";
    }
    ...
    while ($row = mysqli_fetch_assoc($empRecords)) {
        ...
        $data[] = array(
            "name" => '<input type="hidden" class="unit_code_' . $i . '" value="' . $row['unit_code'] . '" />' . $row['name'],
            "active" => $active,
            "action" => $action,
        );
        $i++;
